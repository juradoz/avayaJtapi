   ...
   implements TsapiMonitor
 {
   ...
   private synchronized void deleteReferenceInternal(TSDevice device, int cause, Object privateData)
   {
     ...
     if (this.observer != null)
     {
       ...
       synchronized (this.eventList) {
         ...
         addObserverEvent(new TsapiTermObservationEndedEvent(createTerminal(device), cause, privateData), tsEventLog);
       ...
       if (privateData != null)
       {
         ...
         addObserverEvent(new TsapiPrivateTerminalEvent(createTerminal(device), cause, 136, privateData), tsEventLog);
       }
 
       log.debug("meta event END for " + this.observer + " eventList size=" + this.eventList.size());
     }
     else {
       log.debug("meta event BEGIN: cause (" + cause + ") for " + this.terminalListener);
       tsEventLog = "OBSERVATIONENDEDEVENT for " + device;
 
       this.listenerEventList.addElement(new TerminalEventImpl(121, cause, createTerminal(device), privateData));
 
       log.debug(tsEventLog + " for listener " + this.terminalListener);
       if (privateData != null) {
         tsEventLog = "PRIVATEEVENT for " + device;
         addPrivateDataEvent(603, cause, createTerminal(device), privateData, tsEventLog);
       }
     }
   ...
   void addObserverEvent(TermEv event, String tsEventLog)
   {
     ...
     else if ((this.observerType.get(4)) && (((ITsapiEvent)event).getEventPackage() == 4))
     {
       log.debug(tsEventLog + " for observer " + this.observer);
   ...
   private void addAgentTerminalEvent(TerminalEventParams terminalEventParams, Agent agent, String tsEventLog)
   {
     if (this.terminalListener instanceof AgentTerminalListener) {
       ...
       log.debug(tsEventLog + " ignored since listener " + this.terminalListener + " is not of type AgentTerminalListener");
     }
   }
   ...
   private void addCallControlTerminalEvent(TerminalEventParams terminalEventParams, boolean state, String tsEventLog)
   {
     if (this.terminalListener instanceof CallControlTerminalListener) {
       this.listenerEventList.addElement(new CallControlTerminalEventImpl(terminalEventParams, state));
       log.debug(tsEventLog + " for listener " + this.terminalListener);
     }
     else {
       log.debug(tsEventLog + " ignored since listener " + this.terminalListener + " is not of type CallControlTerminalListener");
     }
   }
   ...
   private void addPrivateDataEvent(int eventId, int cause, Object source, Object privateData, String tsEventLog)
   {
     if (this.terminalListener instanceof PrivateDataTerminalListener) {
       this.listenerEventList.addElement(new PrivateDataEventImpl(eventId, cause, null, source, privateData));
       log.debug(tsEventLog + " for listener " + this.terminalListener);
     }
     else {
       log.debug(tsEventLog + " ignored since listener " + this.terminalListener + " is not of type PrivateDataTerminalListener");
     }
   }
   ...
   private void createObserverEvents(Vector<TSEvent> _eventList, boolean snapshot)
   {
     ...
     for (int i = 0; i < _eventList.size(); ++i) {
       ...
       case 58:
         ...
         addObserverEvent(new TsapiTerminalDNDEvent(createTerminal(target), target.dndState, cause, metaCode, privateData), tsEventLog);
 
         break;
       case 47:
         tsEventLog = "TERMINALLOGGEDONEVENT for " + agent;
 
         addObserverEvent(new TsapiTermLogOnEv(createTerminal(target), createAgent(agent), cause, metaCode, privateData), tsEventLog);
 
         break;
       case 48:
         tsEventLog = "TERMINALLOGGEDOFFEVENT for " + agent;
 
         addObserverEvent(new TsapiTermLogOffEv(createTerminal(target), createAgent(agent), cause, metaCode, privateData), tsEventLog);
 
         break;
       case 49:
         tsEventLog = "TERMINALREADYEVENT for " + agent;
 
         addObserverEvent(new TsapiTermReadyEv(createTerminal(target), createAgent(agent), cause, metaCode, privateData), tsEventLog);
 
         break;
       case 50:
         tsEventLog = "TERMINALNOTREADYEVENT for " + agent;
 
         addObserverEvent(new TsapiTermNotReadyEv(createTerminal(target), createAgent(agent), cause, metaCode, privateData), tsEventLog);
 
         break;
       case 51:
         tsEventLog = "TERMINALWORKREADYEVENT for " + agent;
 
         addObserverEvent(new TsapiTermWorkReadyEv(createTerminal(target), createAgent(agent), cause, metaCode, privateData), tsEventLog);
 
         break;
       case 52:
         tsEventLog = "TERMINALWORKNOTREADYEVENT for " + agent;
 
         addObserverEvent(new TsapiTermWorkNotReadyEv(createTerminal(target), createAgent(agent), cause, metaCode, privateData), tsEventLog);
 
         break;
       case 53:
         tsEventLog = "TERMINALBUSYEVENT for " + agent;
 
         addObserverEvent(new TsapiTermBusyEv(createTerminal(target), createAgent(agent), cause, metaCode, privateData), tsEventLog);
 
         break;
       case 9999:
         tsEventLog = "PRIVATEEVENT for " + target;
 
         addObserverEvent(new TsapiPrivateTerminalEvent(createTerminal(target), cause, metaCode, privateData), tsEventLog);
       }
     ...
     for (int i = 0; i < _eventList.size(); ++i)
     {
       ...
       if ((privateData != null) && (ev.getEventType() != 9999))
         if (!privateData.equals(prevPrivateData)) {
           ...
           tsEventLog = "PRIVATEEVENT for " + target;
           addPrivateDataEvent(603, cause, target, privateData, tsEventLog);
         }
       else {
       ...
       case 58:
         ...
         addCallControlTerminalEvent(terminalEventParams, target.dndState, tsEventLog);
         break;
       case 47:
       ...
       case 9999:
         ...
         addPrivateDataEvent(603, cause, terminal, privateData, tsEventLog);
       }
     }
       ...
       synchronized (this)
       {
         if (this.observer != null)
           ...
           synchronized (this.listenerEventList)
           {
             ...
             listenerEventArray = new Event[this.listenerEventList.size()];
     ...
     for (Event event : listenerEventArray)
       ...
       case 308:
         ...
         try {
           ...
           log.error("EXCEPTION thrown by agentTerminalBusy in " + this.terminalListener + " - " + e.getMessage(), e);
         }
       ...
       case 309:
         ...
         try {
           ...
           log.error("EXCEPTION thrown by agentTerminalLoggedOff in " + this.terminalListener + " - " + e.getMessage(), e);
         }
       ...
       case 310:
         ...
         try {
           ...
           log.error("EXCEPTION thrown by agentTerminalLoggedOn in " + this.terminalListener + " - " + e.getMessage(), e);
         }
       ...
       case 311:
         ...
         try {
           ...
           log.error("EXCEPTION thrown by agentTerminalNotReady in " + this.terminalListener + " - " + e.getMessage(), e);
         }
       ...
       case 312:
         ...
         try {
           ...
           log.error("EXCEPTION thrown by agentTerminalReady in " + this.terminalListener + " - " + e.getMessage(), e);
         }
       ...
       case 313:
         ...
         try {
           ...
           log.error("EXCEPTION thrown by agentTerminalUnknown in " + this.terminalListener + " - " + e.getMessage(), e);
         }
       ...
       case 314:
         ...
         try {
           ...
           log.error("EXCEPTION thrown by agentTerminalWorkNotReady in " + this.terminalListener + " - " + e.getMessage(), e);
         }
       ...
       case 315:
         ...
         try {
           ...
           log.error("EXCEPTION thrown by agentTerminalWorkReady in " + this.terminalListener + " - " + e.getMessage(), e);
         }
       ...
       case 371:
         ...
         try {
           ...
           log.error("EXCEPTION thrown by terminalDoNotDisturb in " + this.terminalListener + " - " + e.getMessage(), e);
         }
       ...
       case 603:
         ...
         try {
           ...
           log.error("EXCEPTION thrown by terminalPrivateData in " + this.terminalListener + " - " + e.getMessage(), e);
         }
