 ...
 final class TSAuditThread extends Thread
 {
   ...
   void dump(String indent)
   {
     ...
     Enumeration callEnum = this.saveCallHash.elements();
...
continue;
     ...
     Enumeration connEnum = this.saveConnHash.elements();
...
continue;
     ...
     Enumeration agentEnum = this.saveAgentHash.elements();
...
continue;
     ...
     log.trace(indent + "TSAuditThread Referenced/NotReferenced dump for TSDevice");
     if (TSDevice.g_RefCnt != TSDevice.g_CreationCnt)
     {
       log.trace(indent + "Reference dump: TSDevice Created Count " + TSDevice.g_CreationCnt);
       log.trace(indent + "Reference dump: TSDevice Referenced Count " + TSDevice.g_RefCnt);
       log.trace(indent + "Reference dump: TSDevice Registered Count " + this.provider.xrefHash.size());
     }
     ...
     try
     {
       while (this.keepRunning)
       {
         ...
         synchronized (this.saveCallHash)
         {
           ...
           Enumeration my_enum = this.saveCallHash.elements();
           while (my_enum.hasMoreElements())
           {
             ...
             try {
               ...
               if ((!sCall.call.needsSnapshot()) && (curTime - sCall.saveTime > 3600000L))
               {
                 log.info("Setting NeedSnapshot for LONG call: " + sCall.call);
...
continue;
             ...
             {
               Object oldObj = keepHash.put(new Integer(sCall.call.getCallID()), sCall);
               if (oldObj != null)
                 log.info("NOTICE: keepHash.put() replaced " + oldObj + " for " + this.provider);
             }
             log.info("AUDIT: removing call " + sCall.call + " for " + this.provider);
           }
         ...
         {
           Hashtable keepHash = new Hashtable(20);
           Enumeration my_enum = this.saveConnHash.elements();
           while (my_enum.hasMoreElements())
           {
             SavedConn sConn;
             try {
...
continue;
             ...
             if (curTime - sConn.saveTime < 5000L)
             {
               try
               {
                 Object oldObj = keepHash.put(sConn.conn.getConnID(), sConn);
                 if (oldObj != null)
                   log.info("NOTICE: keepHash.put() replaced " + oldObj + " for " + this.provider);
               }
               catch (TsapiPlatformException e)
               {
                 ...
                 log.info("AUDIT: removing conn " + sConn.conn + " for " + this.provider);
               }
             }
             log.info("AUDIT: removing conn " + sConn.conn + " for " + this.provider);
           }
         ...
         {
           Hashtable keepHash = new Hashtable(20);
           Enumeration my_enum = this.saveAgentHash.elements();
           while (my_enum.hasMoreElements())
           {
             SavedAgent sAgent;
             try {
...
continue;
             ...
             {
               Object oldObj = keepHash.put(sAgent.agent.getAgentKey(), sAgent);
               if (oldObj != null)
                 log.info("NOTICE: keepHash.put() replaced " + oldObj + " for " + this.provider);
             }
             log.info("AUDIT: removing agent " + sAgent.agent + " for " + this.provider);
           }
         ...
         if (secondsSinceLastProviderDump / 60 < Tsapi.getAuditDumpInterval())
         {
           continue;
         }
   ...
   {
     SavedCall sCall = (SavedCall)this.saveCallHash.remove(new Integer(callID));
     if (sCall != null) {
       log.info("AUDIT (dumpCall): removing call " + sCall.call + " for " + this.provider);
     }
     Hashtable keepHash = new Hashtable(20);
...
continue;
         ...
         try
         {
           if (sConn.conn.getConnID().getCallID() != callID)
           {
             ...
             log.info("AUDIT (dumpCall): removing conn " + sConn.conn + " for " + this.provider);
           }
         }
         catch (TsapiPlatformException e)
         {
   ...
   {
     Object oldObj = this.saveCallHash.put(new Integer(call.getCallID()), new SavedCall(call));
     if (oldObj != null)
       log.info("NOTICE: saveCallHash.put() replaced " + oldObj + " for " + this.provider);
   }
 
   TSCall getCall(int callID) {
     SavedCall sCall = (SavedCall)this.saveCallHash.get(new Integer(callID));
     if (sCall != null)
     {
       log.info("Found call in audit hash: " + sCall.call + " for " + this.provider);
       return sCall.call;
     }
   ...
   void dumpConn(CSTAConnectionID connID) {
     ...
     if (sConn != null)
       log.info("AUDIT (dumpConn): removing conn " + sConn.conn + " for " + this.provider);
   }
   ...
   void putConn(TSConnection conn)
   {
     try {
       ...
       if (connID != null)
       {
         Object oldObj = this.saveConnHash.put(connID, new SavedConn(conn));
         if (oldObj != null)
           log.info("NOTICE: saveConnHash.put() replaced " + oldObj + " for " + this.provider);
       }
     }
     catch (TsapiPlatformException e)
     {
   ...
   TSConnection getConn(CSTAConnectionID connID) {
     ...
     if (sConn != null)
     {
       log.info("Found conn in audit hash: " + sConn.conn + " for " + this.provider);
       return sConn.conn;
     }
   ...
   void dumpAgent(TSAgentKey agentKey) {
     ...
     if (sAgent != null)
       log.info("AUDIT (dumpAgent): removing agent " + sAgent.agent + " for " + this.provider);
   }
   ...
   void putAgent(TSAgent agent) {
     ...
     if (agentKey == null)
       return;
     Object oldObj = this.saveAgentHash.put(agentKey, new SavedAgent(agent));
     if (oldObj != null)
       log.info("NOTICE: saveAgentHash.put() replaced " + oldObj + " for " + this.provider);
   }
   ...
   TSAgent getAgent(TSAgentKey agentKey)
   {
     ...
     if (sAgent != null)
     {
       log.info("Found agent in audit hash: " + sAgent.agent + " for " + this.provider);
       return sAgent.agent;
     }
