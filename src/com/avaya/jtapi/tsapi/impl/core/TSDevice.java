...
public final class TSDevice implements IDomainDevice {
	...
	void dump(String indent) {
		...
		Vector connectionsClone = (Vector) this.connections.clone();
		int i;
		...
		Vector terminalConnectionsClone = (Vector) this.terminalConnections
				.clone();
		...
		Vector tsACDVectorClone = (Vector) this.tsACDVector.clone();
		...
		synchronized (this.tsAgentTermVector) {
			TSAgent agent = null;
			for (i = 0; i < this.tsAgentTermVector.size(); ++i) {
		...
		if (this.sessionHash != null) {
			...
			synchronized (this.sessionHash) {
				Enumeration sessionEnum = this.sessionHash.elements();
		...
		Vector terminalMonitorThreadsClone = (Vector) this.terminalMonitorThreads
				.clone();
		int j;
		...
		Vector addressMonitorThreadsClone = (Vector) this.addressMonitorThreads
				.clone();
		int k;
		...
		Vector callsAtAddressMonitorThreadsClone = (Vector) this.callsAtAddressMonitorThreads
				.clone();
		...
		Vector callsViaAddressMonitorThreadsClone = (Vector) this.callsViaAddressMonitorThreads
				.clone();
		...
		Vector callsAtTerminalMonitorThreadsClone = (Vector) this.callsAtTerminalMonitorThreads
				.clone();
	...
	synchronized void internalRecreate() {
		Vector keys = new Vector(this.devNameVector);
	...
	public String getDirectoryName() {
		...
		try {
			...
			Object lqdnConf = this.provider.sendPrivateData(lqdn
	...
	Vector<TSConnection> getConns() {
		...
		return (Vector) (Vector) this.connections.clone();
	}
	...
	Vector<TSConnection> getTermConns() {
		...
		return (Vector) (Vector) this.terminalConnections.clone();
	}
	...
	Vector<TsapiCallMonitor> getCVDObservers() {
		...
		Vector obs = (Vector) this.callsViaAddressMonitorThreads.clone();
		return obs;
	}
	...
	public void cancelForwarding(CSTAPrivate reqPriv)
		...
		synchronized (this.fwdVector) {
			for (int i = 0; i < this.fwdVector.size(); ++i) {
				TsapiCallControlForwarding fwd = (TsapiCallControlForwarding) this.fwdVector
						.elementAt(i);
				String forwardDN = fwd.getDestinationAddress();
				switch (fwd.getType()) {
				case 2:
	...
	public void removeTSAgent(TSAgent tsAgent, int reasonCode)
		...
		if (tsAgent == null) {
			...
			Vector agentVector = new Vector(this.tsAgentTermVector);
	...
	void snapshotAgentsInACD() {
		...
		try {
			...
			Object lqalConf = this.provider.sendPrivateData(lqal
	...
	void snapshotAgentsInTerminal(String acdName, String agentID) {
		...
		if ((this.provider.isLucent()) && (acdName != null)) {
			...
			if (agent == null) {
				synchronized (this.tsAgentTermVector) {
					if (this.tsAgentTermVector.size() > 0) {
						agent = (TSAgent) this.tsAgentTermVector.elementAt(0);
					}
				}
			}
		...
		if (acdName == null) {
			...
			TSAgentKey currentAgentKey = new TSAgentKey(getName(), acdName,
					agentID);
			if ((agentID != null) && (this.tsAgentTermVector.size() > 0)) {
				TSAgent previousAgent = (TSAgent) this.tsAgentTermVector.get(0);
	...
	void addSession(int routingCrossRefID, TSRouteSession routeSession) {
		...
		Object oldObj = this.sessionHash.put(new Integer(routingCrossRefID),
				routeSession);
		if (oldObj != null)
	...
	boolean doCallSnapshot() {
		...
		conns = new Vector(this.terminalConnections);
		for (int i = 0; i < conns.size(); ++i) {
			TSConnection conn = (TSConnection) conns.elementAt(i);
			if (newCalls.contains(conn.getTSCall()))
				continue;
	...
	void sendTerminalSnapshot(TsapiTerminalMonitor callback) {
		...
		for (int i = 0; i < this.tsAgentTermVector.size(); ++i) {
			((TSAgent) this.tsAgentTermVector.elementAt(i))
	...
	void removeObservers(int cause, Object privateData, int xrefID) {
		...
		Vector observers = new Vector(this.addressMonitorThreads);
		for (int i = 0; i < observers.size(); ++i) {
			removeAddressMonitor((TsapiAddressMonitor) observers.elementAt(i),
					cause, privateData);
		}
		Vector terminalObservers = new Vector(this.terminalMonitorThreads);
	...
	void addConnection(TSConnection tsConn) {
		...
		call
				.addDeviceObservers(
						this,
						(doTerminalObservers) ? this.callsAtTerminalMonitorThreads
								: null,
						(doAddressObservers) ? this.callsAtAddressMonitorThreads
								: null,
						(doAddressObservers) ? this.callsViaAddressMonitorThreads
								: null, true);
	}
