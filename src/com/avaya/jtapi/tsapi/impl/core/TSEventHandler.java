...
import org.apache.log4j.Logger;
   ...
   implements TsapiUnsolicitedHandler
 {
   ...
   public void acsUnsolicited(CSTAEvent event)
   {
     ...
     {
     case 7:
       log.info("Handling ACS_UNIVERSAL_FAILURE event " + event + " for " + this.provider);
     ...
     case 16:
       ...
       break;
     default:
       log.info("WARNING: event " + event.getEventHeader().getEventType() + " not implemented");
     }
   }
   ...
   public void cstaUnsolicited(CSTAEvent event)
   {
     ...
     Object monitored = this.provider.getMonitoredObject(cstaEvent.getMonitorCrossRefID());
     if (monitored == null)
     {
       if (event.getEventHeader().getEventType() != 54)
       {
         log.info("CSTA event " + event + "(" + event.getEventHeader().getEventType() + ") ignored for " + this.provider);
         return;
       }
 
       CSTACallClearedEvent callCleared = (CSTACallClearedEvent)cstaEvent;
       TSCall call = this.provider.createCall(callCleared.getClearedCall().getCallID());
       if (call != null)
       {
         call.getTSProviderImpl().addMonitor(cstaEvent.getMonitorCrossRefID(), call);
       }
     ...
     case 66:
       ...
       doConnEvents(66, monitored, initiated.getCause(), null, initiated.getInitiatedConnection(), 84, 98, privateData, null, null, null, true, 0);
     ...
     case 63:
       ...
       doConnEvents(63, monitored, originated.getCause(), originated.getCallingDevice(), originated.getOriginatedConnection(), 88, 98, privateData, originated.getCallingDevice(), originated.getCalledDevice(), null, true, 0);
     ...
     case 57:
       ...
       doConnEvents(57, monitored, delivered.getCause(), delivered.getAlertingDevice(), delivered.getConnection(), 83, 97, privateData, delivered.getCallingDevice(), delivered.getCalledDevice(), delivered.getLastRedirectionDevice(), false, 0);
     ...
     case 59:
       ...
       doConnEvents(59, monitored, established.getCause(), established.getAnsweringDevice(), established.getEstablishedConnection(), 88, 98, privateData, established.getCallingDevice(), established.getCalledDevice(), established.getLastRedirectionDevice(), false, 0);
     ...
     case 62:
       ...
       doConnEvents(62, monitored, networkReached.getCause(), networkReached.getTrunkUsed(), networkReached.getConnection(), 86, 98, privateData, null, networkReached.getCalledDevice(), null, false, 0);
     ...
     case 65:
       ...
       doConnEvents(65, monitored, retrieved.getCause(), retrieved.getRetrievingDevice(), retrieved.getRetrievedConnection(), 88, 98, privateData, null, null, null, false, 0);
     ...
     case 61:
       ...
       doConnEvents(61, monitored, held.getCause(), held.getHoldingDevice(), held.getHeldConnection(), 88, 99, privateData, null, null, null, false, 0);
     ...
     case 56:
       ...
       doConnEvents(56, monitored, connCleared.getCause(), connCleared.getReleasingDevice(), connCleared.getDroppedConnection(), 89, 102, privateData, null, null, null, false, 0);
     ...
     case 60:
       ...
       doConnEvents(60, monitored, failed.getCause(), failed.getFailingDevice(), failed.getFailedConnection(), 90, 102, privateData, null, failed.getCalledDevice(), null, false, 0);
     ...
     case 119:
       ...
       doMonitorEnded(monitorEnded.getCause(), cstaEvent.getMonitorCrossRefID(), monitored, privateData);
     ...
     case 64:
       ...
       doConnEvents(64, monitored, queued.getCause(), queued.getQueueingDevice(), queued.getQueuedConnection(), 82, -1, privateData, queued.getCallingDevice(), queued.getCalledDevice(), queued.getLastRedirectionDevice(), false, queued.getNumberQueued());
     ...
     case 58:
       ...
       if ((Tsapi.isPatch_enable_PreserveRedirectedVDNAsUNKNOWN()) && (this.provider.getDeviceExt(diverted.getDivertingDevice().getDeviceID()) == 1))
       {
         log.info("Preserving redirecting VDN party of CSTA_DIVERTED for " + this.provider);
 
         doConnEvents(58, monitored, diverted.getCause(), diverted.getDivertingDevice(), diverted.getConnection(), 91, 103, privateData, null, diverted.getNewDestination(), null, false, 0);
       }
       else
       {
         log.info("Dropping party due to handling of CSTA_DIVERTED for " + this.provider);
 
         doConnEvents(58, monitored, diverted.getCause(), diverted.getDivertingDevice(), diverted.getConnection(), 89, 102, privateData, null, diverted.getNewDestination(), null, false, 0);
       }
     ...
     case 55:
       ...
       doConfXfer(207, conferenced.getPrimaryOldCall(), conferenced.getSecondaryOldCall(), conferenced.getConferenceConnections(), privateData, conferenced.getCause());
     ...
     case 67:
       ...
       doConfXfer(212, transferred.getPrimaryOldCall(), transferred.getSecondaryOldCall(), transferred.getTransferredConnections(), privateData, transferred.getCause());
     ...
     case 54:
       ...
       CSTACallClearedEvent callCleared = (CSTACallClearedEvent)cstaEvent;
       doCallEvents(54, monitored, callCleared.getCause(), callCleared.getClearedCall(), 34, privateData);
     ...
     case 72:
       ...
       doAgentEvents(72, monitored, loggedOn.getAgentDevice(), loggedOn.getAgentID(), loggedOn.getAgentGroup(), loggedOn.getPassword(), 1, privateData);
     ...
     case 73:
       ...
       doAgentEvents(73, monitored, loggedOff.getAgentDevice(), loggedOff.getAgentID(), loggedOff.getAgentGroup(), null, 2, privateData);
     ...
     case 74:
       ...
       doAgentEvents(74, monitored, notReady.getAgentDevice(), notReady.getAgentID(), null, null, 3, privateData);
     ...
     case 75:
       ...
       doAgentEvents(75, monitored, ready.getAgentDevice(), ready.getAgentID(), null, null, 4, privateData);
     ...
     case 76:
       ...
       doAgentEvents(76, monitored, workNotReady.getAgentDevice(), workNotReady.getAgentID(), null, null, 5, privateData);
     ...
     case 77:
       ...
       doAgentEvents(77, monitored, workReady.getAgentDevice(), workReady.getAgentID(), null, null, 6, privateData);
     ...
     case 69:
       ...
       doDeviceEvents(69, monitored, doNotDisturb.getDevice(), doNotDisturb.isDoNotDisturbOn(), null, privateData);
     ...
     case 71:
       ...
       doDeviceEvents(71, monitored, messageWaiting.getDeviceForMessage(), messageWaiting.isMessageWaitingOn(), null, privateData);
     ...
     case 70:
       ...
       doDeviceEvents(70, monitored, forwarding.getDevice(), false, forwarding.getForwardingInformation(), privateData);
 
       log.info("DONE handling CSTA_FORWARDING for " + this.provider);
       break;
     case 94:
       log.info("Handling CSTA_PRIVATE_STATUS for " + this.provider);
       if ((monitored instanceof TSCall) || (privateData instanceof LucentEnteredDigitsEvent) || (privateData instanceof LucentChargeAdvice))
       {
         CSTAConnectionID connID = null;
 
         if (privateData instanceof LucentEnteredDigitsEvent)
         {
           connID = ((LucentEnteredDigitsEvent)privateData).getConnection_asn();
         }
         else if (privateData instanceof LucentChargeAdvice)
         {
           connID = ((LucentChargeAdvice)privateData).getConnection_asn();
         }
     ...
     PerfStatisticsCollector.updateUnsolicitedHandlingTime(System.currentTimeMillis() - event.getQueuedTimeStamp());
   }
   ...
   public void cstaRequest(CSTAEvent event)
   {
     ...
     case 130:
       ...
       tsDevice = (TSDevice)this.provider.routeRegHash.get(new Integer(routeRequestExt.getRouteRegisterReqID()));
       if (tsDevice != null)
       {
         ...
         if (tsRouteCallback != null)
         {
           ...
           TSCall call = null;
           currentRouteDevice = this.provider.createDevice(routeRequestExt.getCurrentRoute());
           callingDevice = this.provider.createDevice(routeRequestExt.getCallingDevice());
           call = this.provider.createCall(routeRequestExt.getRoutedCall().getCallID(), privateData);
 
           tsRouteSession = new TSRouteSession(this.provider, tsDevice, routeRequestExt.getRoutingCrossRefID(), currentRouteDevice, callingDevice, call, routeRequestExt.getRoutedSelAlgorithm(), null);
           ...
           if (privateData instanceof LucentRouteRequestEvent)
           {
             ...
             TsapiTrunkImpl tsapiTrunk = TsapiPromoter.promoteTrunk(this.provider, luPrivData.getTrunkGroup(), null);
             ...
             if (privateData instanceof LucentV5RouteRequestEvent)
             {
               ...
               tsRouteSession.setCallOriginatorInfo(luV5PrivData.getCallOriginatorInfo());
               tsRouteSession.setFlexibleBilling(luV5PrivData.isFlexibleBilling());
               ...
               if (privateData instanceof LucentV7RouteRequestEvent)
               {
                 ...
                 tsRouteSession.setDeviceHistory(TsapiPromoter.promoteDeviceHistory(luV7PrivData.getDeviceHistory()));
               }
             }
           }
     ...
     case 83:
       ...
       tsDevice = (TSDevice)this.provider.routeRegHash.get(new Integer(routeRequest.getRouteRegisterReqID()));
       if (tsDevice != null)
       {
         tsRouteCallback = tsDevice.getTSRouteCallback();
         if (tsRouteCallback != null)
         {
           ...
           TSDevice callingDevice = null;
           TSCall call = null;
           currentRouteDevice = this.provider.createDevice(routeRequest.getCurrentRoute());
           callingDevice = this.provider.createDevice(routeRequest.getCallingDevice());
           call = this.provider.createCall(routeRequest.getRoutedCall().getCallID(), privateData);
 
           tsRouteSession = new TSRouteSession(this.provider, tsDevice, routeRequest.getRoutingCrossRefID(), currentRouteDevice, callingDevice, call, routeRequest.getRoutedSelAlgorithm(), null);
     ...
     case 85:
       ...
       tsDevice = (TSDevice)this.provider.routeRegHash.get(new Integer(reRouteRequest.getRouteRegisterReqID()));
       if (tsDevice != null)
       {
         tsRouteCallback = tsDevice.getTSRouteCallback();
         if ((tsRouteCallback != null) && (tsDevice.sessionHash != null))
         {
           tsRouteSession = (TSRouteSession)tsDevice.sessionHash.get(new Integer(reRouteRequest.getRoutingCrossRefID()));
           if (tsRouteSession != null)
           {
             TSEvent tsapiEvent = tsRouteSession.setState(4);
             if (tsapiEvent != null)
             {
               log.info("REROUTEEVENT for " + tsDevice + " for " + this.provider);
               tsRouteCallback.deliverEvent(tsapiEvent);
             }
           }
         }
   ...
   public void cstaEventReport(CSTAEvent event)
   {
     ...
     case 131:
       ...
       tsDevice = (TSDevice)this.provider.routeRegHash.get(new Integer(routeUsedEventExt.getRouteRegisterReqID()));
       if (tsDevice != null)
       {
         tsRouteCallback = tsDevice.getTSRouteCallback();
         if ((tsRouteCallback != null) && (tsDevice.sessionHash != null))
         {
           tsRouteSession = (TSRouteSession)tsDevice.sessionHash.get(new Integer(routeUsedEventExt.getRoutingCrossRefID()));
           if (tsRouteSession != null)
           {
             routeUsedDevice = this.provider.createDevice(routeUsedEventExt.getRouteUsed());
             callingDevice = this.provider.createDevice(routeUsedEventExt.getCallingDevice());
             ...
             if (tsapiEvent != null)
             {
               log.info("ROUTEUSEDEVENT for " + tsDevice + " for " + this.provider);
               tsRouteCallback.deliverEvent(tsapiEvent);
             }
           }
         }
     ...
     case 86:
       ...
       tsDevice = (TSDevice)this.provider.routeRegHash.get(new Integer(routeUsedEvent.getRouteRegisterReqID()));
       if (tsDevice != null)
       {
         tsRouteCallback = tsDevice.getTSRouteCallback();
         if ((tsRouteCallback != null) && (tsDevice.sessionHash != null))
         {
           tsRouteSession = (TSRouteSession)tsDevice.sessionHash.get(new Integer(routeUsedEvent.getRoutingCrossRefID()));
           if (tsRouteSession != null)
           {
             routeUsedDevice = this.provider.createDevice(routeUsedEvent.getRouteUsed());
             callingDevice = this.provider.createDevice(routeUsedEvent.getCallingDevice());
             tsRouteSession.setRouteUsedDevice(routeUsedDevice);
             tsRouteSession.setCallingDevice(callingDevice);
     ...
     case 87:
       ...
       tsDevice = (TSDevice)this.provider.routeRegHash.get(new Integer(routeEndEvent.getRouteRegisterReqID()));
       if (tsDevice != null)
       {
         tsRouteCallback = tsDevice.getTSRouteCallback();
         if ((tsRouteCallback != null) && (tsDevice.sessionHash != null))
         {
           tsRouteSession = (TSRouteSession)tsDevice.sessionHash.get(new Integer(routeEndEvent.getRoutingCrossRefID()));
           if (tsRouteSession != null)
           {
             ...
             if (tsapiEvent != null)
             {
               log.info("ROUTEENDEVENT for " + tsDevice + " for " + this.provider);
               tsRouteCallback.deliverEvent(tsapiEvent);
             }
           }
         }
     ...
     case 82:
       ...
       tsDevice = (TSDevice)this.provider.routeRegHash.get(new Integer(registerAbortEvent.getRouteRegisterReqID()));
       if (tsDevice != null)
       {
         tsRouteCallback = tsDevice.getTSRouteCallback();
         if (tsRouteCallback != null)
     ...
     case 93:
       ...
       if (replyPriv instanceof LucentQueryAgentLoginResp)
       {
         ...
         tsDevice = this.provider.findACDDevice(respEvent.getPrivEventCrossRefID());
         if (tsDevice != null)
         {
     ...
     case 106:
       ...
       if ((sysStat.getState() == 6) || (sysStat.getState() == 5) || (sysStat.getState() == 4))
       {
       ...
       for (int j = 0; j < monitors.size(); ++j) {
         TsapiProviderMonitor callback = (TsapiProviderMonitor)monitors.elementAt(j);
   ...
   void doConnEvents(int eventType, Object monitored, short cause, CSTAExtendedDeviceID subjectDeviceID, CSTAConnectionID connID, int connState, int termConnState, Object privateData, CSTAExtendedDeviceID callingDeviceID, CSTAExtendedDeviceID calledDeviceID, CSTAExtendedDeviceID lastRedirectionDeviceID, boolean dontNeedSnapshot, int numQueued)
   {
     ...
     TSDevice subjectDevice = this.provider.createDevice(subjectDeviceID, connID);
     if (subjectDevice == null)
     {
       if ((((subjectDeviceID == null) || (subjectDeviceID.getDeviceIDStatus() == 2))) && (monitored instanceof TSDevice))
       {
         if (this.provider.isLucent())
         {
           subjectDevice = this.provider.createDevice(connID.getDeviceID(), connID);
         }
         else
         {
     ...
     if (cause == 17)
     {
       ...
       case 63:
         if (privateData instanceof LucentOriginatedEvent)
         {
           ...
           try {
             physicalTerminal = TsapiPromoter.promoteTerminal(this.provider, luPrivData.getPhysicalTerminal_asn());
           }
     ...
     call = this.provider.validateCall(privateData, call, connID.getCallID());
 
     call.setCSTACause(cause);
 
     if ((monitored != null) && (monitored instanceof TSDevice) && (((TSDevice)monitored).getDeviceType() == 1))
     {
     ...
     case 57:
       ...
       {
         TSConnection dumpingConn = this.provider.createTerminalConnection(connID, subjectDevice, eventList, connectionDevice);
         ...
         if ((((oldCState == 89) || (oldTCState == 102))) && ((
           ((!dumpingConn.isTerminalConnection()) && (connState != 89)) || ((dumpingConn.isTerminalConnection() == true) && (termConnState != 102)))))
         {
     ...
     if (!dontNeedSnapshot)
     {
       ...
       {
         SnapshotCallExtraConfHandler handler = new UnsolicitedSnapshotCallConfHandler(this, eventType, cause, subjectDeviceID, subjectDevice, connID, call, privateData, callingDeviceID, calledDeviceID, lastRedirectionDeviceID, connState, termConnState, connectionDevice);
     ...
     TSConnection connection = this.provider.createTerminalConnection(connID, subjectDevice, eventList, connectionDevice);
     ...
     if ((oldConnState == 89) || (oldTermConnState == 102))
     {
       ...
       case 66:
         ...
         call = this.provider.createCall(connID.getCallID());
         call.setNeedSnapshot(false);
         connection = this.provider.createTerminalConnection(connID, subjectDevice, eventList, connectionDevice);
 
         oldConnState = connection.getCallControlConnState();
         oldTermConnState = connection.getCallControlTermConnState();
         break;
       default:
         if (((!connection.isTerminalConnection()) && (connState != 89)) || ((connection.isTerminalConnection() == true) && (termConnState != 102)))
         {
           eventList = new Vector();
     ...
     if (eventType == 56)
     {
       ...
       if (allConnections.size() <= 2)
       {
         ...
         {
           TSConnection tmpconn = (TSConnection)allConnections.elementAt(i);
           if ((tmpconn == connection) || (
             (tmpconn.getTSConnState() != 53) && (tmpconn.getTSConnState() != 52))) {
             continue;
           }
           call.setState(34, eventList);
         }
       ...
       if (allConnections.size() >= 1)
       {
         ...
         if (allConnections.equals(listOfConnsBelongToDiffDevIDType))
         {
           for (int i = 0; i < allConnections.size(); ++i)
           {
             ...
             if (tmpconn.isDoNotExpectConnectionClearedEvent())
             {
               log.info("Conn " + tmpconn + ", has 'connBelongToDifferentDeviceIDType' flag set. Clearing connection.");
     ...
     finishConnEvents(monitored, eventType, cause, jtapiCause, subjectDeviceID, subjectDevice, connection, call, privateData, callingDeviceID, calledDeviceID, lastRedirectionDeviceID, connState, oldConnState, oldTermConnState, eventList);
   ...
   {
     if ((calling != null) && (called != null) && (calling.getDeviceIDStatus() == 0) && (called.getDeviceIDStatus() == 0))
     {
   ...
   void finishConnEvents(Object monitored, int eventType, int cause, int jtapiCause, CSTAExtendedDeviceID subjectDeviceID, TSDevice subjectDevice, TSConnection connection, TSCall call, Object privateData, CSTAExtendedDeviceID callingDeviceID, CSTAExtendedDeviceID calledDeviceID, CSTAExtendedDeviceID lastRedirectionDeviceID, int connState, int oldConnState, int oldTermConnState, Vector<TSEvent> eventList)
   {
     ...
     TSDevice lastRedirectionDevice = this.provider.createDevice(lastRedirectionDeviceID);
     ...
     if ((eventType == 62) && (subjectDeviceID != null))
     {
       ...
       if (privateData instanceof LucentV5NetworkProgressInfo)
       {
         ...
         if (luPrivData != null)
         {
           TsapiTrunkImpl tsapiTrunk = TsapiPromoter.promoteTrunk(this.provider, luPrivData.getTrunkGroup(), luPrivData.getTrunkMember());
     ...
     if (privateData instanceof LucentDeliveredEvent)
     {
       ...
       LucentACDAddressImpl acdAddr = TsapiPromoter.promoteACDAddress(this.provider, ((LucentDeliveredEvent)privateData).getSplit_asn());
       ...
       TsapiAddress address = TsapiPromoter.promoteDeviceIDToAddress(this.provider, luPrivData.getDistributingDevice_asn());
       ...
       if (tsapiTrunk != null)
       {
         ...
         if (!this.provider.isDeviceMonitorable(subjectDeviceID.getDeviceID()))
         {
           trk.setType(2);
         }
       ...
       if (privateData instanceof LucentV5DeliveredEvent)
       {
         ...
         if (privateData instanceof LucentV7DeliveredEvent)
         {
           ...
           call.setDeviceHistory(TsapiPromoter.promoteDeviceHistory(luV7PrivData.getDeviceHistory()));
 
           if (luV7PrivData.getDistributingVDN_asn() != null)
           {
             distributingVDN = TsapiPromoter.promoteExtendedDeviceIDToTSDevice(this.provider, luV7PrivData.getDistributingVDN_asn());
     ...
     else if (privateData instanceof LucentEstablishedEvent)
     {
       ...
       LucentACDAddressImpl acdAddr = TsapiPromoter.promoteACDAddress(this.provider, luPrivData.getSplit_asn());
       ...
       if (tsapiTrunk != null)
       {
         ...
         TSConnection trkConn = (isCalledBetterEndpoint(callingDeviceID, calledDeviceID)) ? otherConn : connection;
     ...
     else if (privateData instanceof LucentFailedEvent)
     {
       ...
       if (privateData instanceof LucentV8FailedEvent)
       {
         ...
         if (luV8PrivData.getCallingDevice() != null) {
           call.setCallingDevices(TsapiPromoter.promoteExtendedDeviceIDToTSDevice(this.provider, luV8PrivData.getCallingDevice()));
         }
     ...
     if ((eventType == 57) && (getJtapiCause(cause) == 210))
     {
       ...
       dropDevices[2] = ((distributingVDN == null) ? distributingDevice : distributingVDN);
       for (int j = 0; j < 3; ++j)
       {
         if ((dropDevices[j] == null) || (
           (dropDevices[j].getDeviceType() != 2) && (dropDevices[j].getDeviceType() != 1)))
           continue;
     ...
     if ((eventType == 64) && (cause == 28) && (lastRedirectionDevice != null))
     {
       ...
       if (oldConnState != 83)
       {
         TSConnection lastRedirConn = call.getConnAtDevice(lastRedirectionDevice);
         if (lastRedirConn != null)
         {
           ...
           {
             TSConnection acdManagerConn = lastRedirConn.getACDManagerConn();
     ...
     if ((monitored != null) && (eventType == 58) && (monitored instanceof TSDevice) && (((TSDevice)monitored).getDeviceType() == 1))
     {
   ...
   void doCallEvents(int eventType, Object monitored, short cause, CSTAConnectionID connID, int callState, Object privateData)
   {
     ...
     if (eventType == 94)
     {
       if (privateData instanceof LucentEnteredDigitsEvent)
       {
         ...
         LucentCallImpl connection = TsapiPromoter.promoteConnection(this.provider, luPrivData.getConnection_asn());
   ...
   void doCallMonitors(TSCall call, Vector<TSEvent> eventList, int jtapiCause, Object privateData)
   {
     ...
     if (privateData != null)
     {
       ...
       if (!this.provider.isLucent()) {
         eventList.addElement(new TSEvent(9999, call, privateData, this.provider));
       }
       else if (privateData instanceof LucentChargeAdvice)
       {
         eventList.addElement(new TSEvent(9999, call, privateData, this.provider));
   ...
   void doAgentEvents(int eventType, Object monitored, CSTAExtendedDeviceID subjectDeviceID, String agentID, String agentGroup, String agentPassword, int agentState, Object privateData)
   {
     ...
     TSAgentKey agentKey = new TSAgentKey(subjectDevice.getName(), agentGroup, agentID);
     TSAgent agent = this.provider.createAgent(agentKey, agentGroup, agentPassword, TSProviderImpl.CREATEAGENT_REFUSE_DELETED);
...
//       {
//         reasonCode = ((LucentAgentModeChangeEvent)privateData).getReasonCode();
//       }
//     }
   ...
   void doAgentMonitors(TSAgent agent, Vector<TSEvent> eventList, Object privateData)
   {
     ...
     if (privateData != null)
     {
       ...
       if (!this.provider.isLucent()) {
         eventList.addElement(new TSEvent(9999, agent, privateData, this.provider));
       }
     }
     Vector observers = null;
   ...
   void doDeviceMonitors(TSDevice device, Vector<TSEvent> eventList, Object privateData)
   {
     ...
     if (privateData != null)
     {
       ...
       if (!this.provider.isLucent()) {
         eventList.addElement(new TSEvent(9999, device, privateData, this.provider));
       }
     }
   ...
   void doMonitorEnded(int cause, int xrefID, Object monitored, Object privateData)
   {
     ...
     if (monitored instanceof TSDevice)
     {
       ((TSDevice)monitored).removeObservers(jtapiCause, privateData, xrefID);
     } else {
       if (!(monitored instanceof TSCall))
         return;
       ((TSCall)monitored).removeObservers(jtapiCause, privateData, xrefID);
     }
   }
   ...
   void doConfXfer(int jtapiCause, CSTAConnectionID primaryConnID, CSTAConnectionID secondaryConnID, CSTAConnection[] connList, Object privateData, short cause)
   {
     ...
     if ((this.provider.isLucentV5()) && (primaryConnID != null) && (primaryConnID.getCallID() == 0))
     {
       primaryConnID = null;
     }
     if ((this.provider.isLucentV5()) && (secondaryConnID != null) && (secondaryConnID.getCallID() == 0))
     {
     ...
     {
       call = this.provider.createCall(connList[0].getParty().getCallID(), privateData);
 
       call = this.provider.validateCall(privateData, call, connList[0].getParty().getCallID());
 
       if (primaryConnID == null)
       {
         if ((secondaryConnID != null) && (connList[0].getParty().getCallID() != secondaryConnID.getCallID()) && 
           (call.getTSState() == 33))
         {
           return;
         }
       ...
       {
         if ((primaryConnID != null) && (connList[0].getParty().getCallID() != primaryConnID.getCallID()) && 
           (call.getTSState() == 33))
         {
           return;
         }
       ...
       else if ((connList[0].getParty().getCallID() != primaryConnID.getCallID()) && (connList[0].getParty().getCallID() != secondaryConnID.getCallID()) && 
         (call.getTSState() == 33))
       {
         return;
       }
     ...
     if (primaryConnID != null)
     {
       call = this.provider.createCall(primaryConnID.getCallID(), privateData);
 
       call = this.provider.validateCall(privateData, call, primaryConnID.getCallID());
     }
 
     if (secondaryConnID != null)
     {
       secCall = this.provider.createCall(secondaryConnID.getCallID(), privateData);
 
       secCall = this.provider.validateCall(privateData, secCall, secondaryConnID.getCallID());
     }
     ...
     if ((call != null) && (call.getTSState() == 34) && (secCall != null) && (secCall.getTSState() == 34))
     {
       return;
     }
     ...
     {
       log.info("a snapshot call is pending for call " + secCall + " call id =" + secCall.callID);
       swapCalls = true;
     }
     else if ((call != null) && (call.getCallObservers().size() == 0) && (secCall != null) && (secCall.getCallObservers().size() > 0))
     {
       swapCalls = true;
     }
     else if ((call == null) || (call.getCallObservers().size() <= 0) || (secCall == null) || (secCall.getCallObservers().size() != 0))
       ...
       if ((call == null) || (secCall != null))
       {
         ...
         else if ((secCall == null) || (secCall.getTSState() != 34))
         {
           ...
           else if ((call == null) && (secCall != null))
           {
             ...
             if ((secCall != null) && (secCall.getCallID() == connList[0].getParty().getCallID()))
             {
               swapCalls = true;
             }
           }
     ...
     {
       boolean found = false;
       device = this.provider.createDevice(connList[i].getStaticDevice(), connList[i].getParty());
       if (device == null) {
         continue;
       }
         ...
         for (int j = 0; j < oldConnections.size(); ++j)
         {
           ...
           if ((cv != null) && (cv.size() > 0))
           {
             ...
             for (int k = 0; k < termConns.size(); ++k)
             {
               ...
               tc.setTermConnState(103, null); break;
             }
           ...
           else if (conn.getTSDevice() == device)
           {
             ...
             {
               log.error("TSEventHandler.doConfXfer() caught TsapiPlatformException from setConnID() while processing connList, i=" + i + ", j=" + j + ", party=" + connList[i].getParty());
               log.error(e.getMessage(), e);
             ...
             conn.setTermConnState(103, null); break;
           }
       ...
       conn = this.provider.createTerminalConnection(connList[i].getParty(), device, tempEventList, device);
         ...
         for (int j = 0; j < oldSecConnections.size(); ++j)
         {
           ...
           if ((cv != null) && (cv.size() > 0))
           {
             ...
             for (int k = 0; k < termConns.size(); ++k)
             {
               ...
               conn.setConnectionState(tc.getCallControlConnState(), priEventList);
               conn.setTermConnState(tc.getCallControlTermConnState(), priEventList);
               found = true;
               break;
             }
           ...
           else if (conn.getTSDevice() == secConn.getTSDevice())
           {
             ...
             conn.setConnectionState(secConn.getCallControlConnState(), priEventList);
             conn.setTermConnState(secConn.getCallControlTermConnState(), priEventList);
             found = true;
             break;
           }
     ...
     {
       CSTATrunkInfo[] trunkList = ((LucentTrunkConnectionMapping)privateData).getLucentTrunkInfo();
 
       LucentTrunkInfoMapItem[] trunkMapItems = LucentTrunkInfoMapItem.createLucentTrunkInfoMapItemArray(trunkList, this.provider);
     ...
     if (snapConnections.size() > 0)
     {
       ...
       SnapshotCallExtraConfHandler handler = new XferConfSnapshotCallConfHandler(this, call, jtapiCause, privateData, snapConnections);
 
       call.doSnapshot(((TSConnection)snapConnections.elementAt(0)).getConnID(), handler, false);
     }
     ...
     if ((jtapiCause == 212) || (jtapiCause == 207)) {
       ...
       for (Object ev : eventList) {
         ((TSEvent) ev).setTransferredEventParams(transferredEventParams);
       }
     }
     doCallMonitors(call, eventList, jtapiCause, privateData);
