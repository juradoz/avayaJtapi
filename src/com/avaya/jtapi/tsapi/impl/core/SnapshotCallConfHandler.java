   ...
   implements ConfHandler
 {
   ...
   void addExtraHandler(SnapshotCallExtraConfHandler _extraHandler)
   {
     ...
     {
       this.extraHandlerVector = new Vector();
     }
   ...
   public void handleConf(CSTAEvent event)
   {
     try
     {
       ...
       try
       {
         ...
         {
           TSErrorMap.throwCSTAException(((CSTAUniversalFailureConfEvent)event.getEvent()).getError());
         }
         ...
         {
           throw new TsapiPlatformException(4, 1, "expected CSTASnapshotCallConfEvent");
         }
       }
       ...
       CSTASnapshotCallResponseInfo[] info = ((CSTASnapshotCallConfEvent)event.getEvent()).getSnapshotData();
       ...
       for (int i = 0; i < info.length; ++i)
       {
         try
         {
           ...
           device = this.call.getTSProviderImpl().createDevice(info[i].getDeviceOnCall(), info[i].getCallIdentifier());
           ...
           if (device.isForExternalDeviceMatchingLocalExtensionNumber(extDevID)) {
             connection = this.call.getTSProviderImpl().createConnection(info[i].getCallIdentifier(), device, null);
             connection.setDoNotExpectConnectionClearedEvent(true);
           }
           else {
             connection = this.call.getTSProviderImpl().createTerminalConnection(info[i].getCallIdentifier(), device, null, device);
           }
           ...
           {
             log.info("SnapshotCallConfHandler.handleConf(): recreating connection " + connection + "; oldConnState=" + oldConnState + ", oldTermConnState=" + oldTermConnState);
 
             connection.delete();
             this.call.getTSProviderImpl().dumpConn(info[i].getCallIdentifier());
         ...
         connection.setStateFromLocalConnState(info[i].getLocalConnectionState());
       ...
       if (this.call.confController != null)
       {
         synchronized (connections)
         {
           for (int i = 0; i < connections.size(); ++i)
           {
             ...
             if ((termConns == null) || (!termConns.contains(this.call.confController)))
               continue;
             found = true;
             break;
           }
       ...
       if (this.call.xferController != null)
       {
         synchronized (connections)
         {
           for (int i = 0; i < connections.size(); ++i)
           {
             TSConnection conn = (TSConnection)connections.elementAt(i);
             Vector termConns = conn.getTermConns();
             if ((termConns == null) || (!termConns.contains(this.call.xferController)))
               continue;
             found = true;
             break;
           }
       ...
       try
       {
         if (this.extraHandlerVector != null)
         {
           ...
           for (int i = 0; i < this.extraHandlerVector.size(); ++i)
           {
             try
             {
               SnapshotCallExtraConfHandler extraHandler = (SnapshotCallExtraConfHandler)this.extraHandlerVector.elementAt(i);
 
               Object pd = extraHandler.handleConf(this.rc, eventList, privateData);
               if (pd != null)
                 privateData = pd;
             } catch (Exception e) {
             }
