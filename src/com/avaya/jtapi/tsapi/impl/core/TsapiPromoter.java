 ...
 public final class TsapiPromoter
 {
   static V7DeviceHistoryEntry[] promoteDeviceHistory(LucentDeviceHistoryEntry[] array)
   {
     ...
     for (int i = 0; i < array.length; ++i)
     {
       ...
       local[i] = new V7DeviceHistoryEntryImpl(array[i].getOldDeviceID(), array[i].getEventCause(), new ConnectionID(cid.getCallID(), cid.getDeviceID(), (short)cid.getDevIDType()));
     }
   ...
   static TsapiTrunkImpl promoteTrunk(TSProviderImpl provider, String trunkGroup, String trunkMember, int type)
   {
     ...
     if (trkName != null)
     {
       ...
       if (tsTrunk != null)
       {
         ...
         return (TsapiTrunkImpl)TsapiCreateObject.getTsapiObject(tsTrunk, false);
       }
     }
     return null;
   }
   ...
   {
     return JtapiOriginalCallInfoFactory.createOriginalCallInfo(provider, oci_csta);
   }
   ...
   static UserEnteredCode promoteUserEnteredCode(TSProviderImpl provider, LucentUserEnteredCode csta_obj)
   {
     if (csta_obj != null)
     {
       ...
       try
       {
         ...
         {
           uec.setCollectVDN(new LucentACDManagerAddressImpl(provider, csta_obj.getCollectVDN_asn()));
         }
       }
       catch (TsapiInvalidArgumentException e) {
       }
   ...
   public static Object promotePrivateEvent(TSProviderImpl provider, Object csta_obj)
   {
     ...
     {
       return promoteNetworkProgressInfo(provider, (LucentNetworkProgressInfo)csta_obj);
     }
     ...
     {
       return promoteLucentConferencedEvent(provider, (LucentConferencedEvent)csta_obj);
     }
     ...
     {
       return promoteLucentDeliveredEvent(provider, (LucentDeliveredEvent)csta_obj);
     }
 
     if (csta_obj instanceof LucentEstablishedEvent)
     {
       return promoteLucentEstablishedEvent(provider, (LucentEstablishedEvent)csta_obj);
     }
 
     if (csta_obj instanceof LucentTransferredEvent)
     {
       return promoteLucentTransferredEvent(provider, (LucentTransferredEvent)csta_obj);
     }
   ...
   private static PrivateDataParams promoteLucentConferencedEvent(TSProviderImpl provider, LucentConferencedEvent csta_obj)
   {
     ...
     params.setDistributingDevice(promoteDeviceIDToAddress(provider, csta_obj.getDistributingDevice_asn()));
 
     params.setOriginalCallInfo(promoteOriginalCallInfo(provider, csta_obj.getOriginalCallInfo()));
   ...
   private static PrivateDataParams promoteLucentDeliveredEvent(TSProviderImpl provider, LucentDeliveredEvent csta_obj)
   {
     ...
     params.setLookaheadInfo(promoteLookaheadInfo(csta_obj.getLookaheadInfo()));
     params.setOriginalCallInfo(promoteOriginalCallInfo(provider, csta_obj.getOriginalCallInfo()));
 
     params.setReason(csta_obj.getReason());
     params.setSplit(promoteACDAddress(provider, csta_obj.getSplit_asn()));
     params.setTrunk(promoteTrunk(provider, csta_obj.getTrunkGroup(), csta_obj.getTrunkMember()));
     params.setUserEnteredCode(promoteUserEnteredCode(provider, csta_obj.getUserEnteredCode()));
     params.setUserToUserInfo(promoteUserToUserInfo(csta_obj.getUserInfo()));
     ...
     if (csta_obj instanceof LucentV5DeliveredEvent) {
       ...
       if (info != null) {
         ...
         params.setCallOriginatorType(v5Obj.getCallOriginatorInfo().getCallOriginatorType());
       }
     }
     return params;
   }
   ...
   {
     return new CSTAPrivate(jtapi_obj.vendor, jtapi_obj.data, jtapi_obj.tsType);
   }
   ...
   static LucentChargeAdviceEvent promoteChargeAdvice(TSProviderImpl provider, LucentChargeAdvice csta_obj)
   {
     ...
     if (calledDevice_asn != null)
     {
       ...
       calledDevice = (TsapiAddress)TsapiCreateObject.getTsapiObject(device, true);
     }
     if (chargingDevice_asn != null)
     {
       TSDevice device = provider.createDevice(chargingDevice_asn);
       chargingDevice = (TsapiAddress)TsapiCreateObject.getTsapiObject(device, true);
     }
 
     trunk = promoteTrunk(provider, csta_obj.getTrunkGroup(), csta_obj.getTrunkMember(), 2);
 
     return new LucentChargeAdviceEventImpl(csta_obj.getChargeType(), csta_obj.getCharge(), call, calledDevice, chargingDevice, csta_obj.getChargeError(), trunk);
   }
 
   static NetworkProgressInfo promoteNetworkProgressInfo(TSProviderImpl provider, LucentNetworkProgressInfo csta_obj)
   {
     return JtapiNetworkProgressInfoFactory.createNetworkProgressInfo(provider, csta_obj);
   }
 }
