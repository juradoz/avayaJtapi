   ...
   implements ConfHandler, HandleConfOnCurrentThread
 {
   ...
   public void handleConf(CSTAEvent event)
   {
     ...
     if (event.getEvent() instanceof CSTASysStatStartConfEvent) {
       log.info("Filter set at " + SystemStatusFilter.print(((CSTASysStatStartConfEvent)event.getEvent()).getFilter(), "", ""));
 
       if (event.getPrivData() instanceof LucentLinkStatusEvent) {
         short state = ((LucentLinkStatusEvent)event.getPrivData()).getLinkStatus().getLinkState();
       ...
       this.systemStatus = ((CSTASysStatReqConfEvent)event.getEvent()).getSystemStatus();
     }
   }
