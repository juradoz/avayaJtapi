...
import org.apache.log4j.Logger;
 ...
 public final class TSAgent
 {
   ...
   {
     updateState(_state, _workMode, _reasonCode, _pendingState, _pendingReasonCode, -1, eventList);
   }
   ...
   void updateState(int _state, int _workMode, int _reasonCode, int _pendingState, int _pendingReasonCode, int _lucentworkmode, Vector<TSEvent> eventList)
   {
     ...
     synchronized (this)
     {
       if (((this.state == _state) && (this.workMode == _workMode) && (this.reasonCode == _reasonCode) && (this.pendingState == _pendingState) && (this.pendingReasonCode == _pendingReasonCode) && (this.lucentWorkMode == _lucentworkmode)) || (this.state == 2))
       {
         return;
       }
       ...
       if (eventList == null)
       {
         if (localEventList.size() > 0)
         {
           ...
           else
           {
             ...
             {
               TSDevice skillDevice = (TSDevice)this.skillsVector.elementAt(i);
               observers = skillDevice.getAddressObservers();
               for (int j = 0; j < localEventList.size(); ++j)
               {
                 TSEvent ev = (TSEvent)localEventList.elementAt(j);
           ...
           {
             TsapiTerminalMonitor callback = (TsapiTerminalMonitor)tObservers.elementAt(j);
             callback.deliverEvents(localEventList, false);
           }
         }
       }
     ...
     else
     {
       for (int i = 0; i < this.skillsVector.size(); ++i)
       {
         ((TSDevice)this.skillsVector.elementAt(i)).removeFromACDVector(this);
       }
     }
     delete();
   }
   ...
   private LucentSetAgentState createLucentSetAgentState(int workMode, int reasonCode, boolean enablePending)
   {
     if (this.provider.isLucentV6())
       return new LucentV6SetAgentState((short)workMode, reasonCode, enablePending);
     ...
     throws TsapiInvalidArgumentException, TsapiInvalidStateException
   {
     ...
     if (((this.state == _state) && (this.workMode == _workMode) && (this.reasonCode == _reasonCode)) || (this.state == 2))
     {
       return false;
     }
 
     if ((_state != 1) && (_state != 2) && (_state != 4) && (_state != 3) && (_state != 6) && (_state != 5))
     {
     ...
     if (this.provider.isLucent())
     {
       ...
       if (_state == 3)
       {
         lsas = createLucentSetAgentState(-1, _reasonCode, _enablePending);
     ...
     if (this.acdDevice != null)
       this.agentDevice.setTSAgent(this.acdDevice.getName(), _state, this.agentID, this.passwd, reqPriv);
     else {
       this.agentDevice.setTSAgent(null, _state, this.agentID, this.passwd, reqPriv);
     }
     if (this.agentDevice.wereChangesHeldPending())
     {
       updateState(this.state, this.workMode, this.reasonCode, _state, effectivePendingReasonCode, null);
     }
     else
     {
   ...
   public int getState()
   {
     ...
     if (this.provider.getCapabilities().getQueryAgentState() != 0)
     {
       ...
       {
         LucentQueryAgentState lqas = new LucentQueryAgentState(this.acdDevice.getName());
       ...
       try
       {
         this.provider.tsapi.queryAgentState(this.agentDevice.getName(), priv, handler);
       }
       ...
       catch (Exception e)
       {
         if (e instanceof ITsapiException) {
           throw new TsapiPlatformException(((ITsapiException)e).getErrorType(), ((ITsapiException)e).getErrorCode(), "queryAgentState failure");
         }
   ...
   boolean monitorIsSet()
   {
     if (this.acdDevice != null)
     {
       ...
       for (int i = 0; i < this.skillsVector.size(); ++i)
       {
         if (((TSDevice)this.skillsVector.elementAt(i)).getAddressObservers().size() > 0) {
           return true;
         }
       }
     }
   ...
   public LucentAgentStateInfoEx getStateInfo()
   {
     ...
     {
       if (this.provider.isLucentV6()) {
         return new LucentV6AgentStateInfoEx(this.state, this.workMode, this.reasonCode, this.pendingState, this.pendingReasonCode, this.lucentWorkMode);
       }
       return new LucentV5AgentStateInfo(this.state, this.workMode, this.reasonCode);
     }
   ...
   void addToSkillsVector(String acdDeviceID)
   {
     ...
     this.skillsVector.addElement(this.provider.createDevice(acdDeviceID));
   ...
   {
     return "TSAgent" + getMyCustomString() + "@" + Integer.toHexString(super.hashCode());
   }
