...
class TsapiAgent implements ITsapiAgent {
	...
	TsapiAgent(final TSAgent _tsAgent) {
		tsAgent = _tsAgent;
		if (tsAgent != null)
			tsAgent.referenced();
	...
	public final AgentTerminal getAgentTerminal() {
		...
		final TSDevice tsDevice = tsAgent.getTSAgentDevice();
		if (tsDevice != null) {
			final AgentTerminal term = (AgentTerminal) TsapiCreateObject
	...
	public final LucentAgentStateInfoEx getStateInfo() {
		...
		final LucentAgentStateInfoEx state = tsAgent.getStateInfo();
	...
	public void setState(final int state, final int workMode)
			throws TsapiInvalidArgumentException, TsapiInvalidStateException {
	...
	public void setState(final int state, final int workMode,
			final int reasonCode) throws TsapiInvalidArgumentException,
	...
	public boolean setState(final int state, final int workMode,
		...
		final boolean done = tsAgent.setState(state, workMode, reasonCode,
				enablePending);
		TsapiTrace
				.traceExit(
						"setState[int state, int workMode, int reasonCode, boolean enablePending]",
