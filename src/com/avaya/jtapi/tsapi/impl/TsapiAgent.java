   ...
   implements ITsapiAgent
 {
     ...
     throws TsapiInvalidArgumentException, TsapiInvalidStateException
   {
     ...
     {
       throw new TsapiInvalidArgumentException(3, 0, "state '" + state + "' not valid. Valid states are Agent.READY, Agent.NOT_READY" + ", Agent.WORK_READY and Agent.WORK_NOT_READY");
     }
 
     if ((workMode != 0) && (workMode != 1) && (workMode != 2))
     {
       throw new TsapiInvalidArgumentException(3, 0, "workMode '" + workMode + "' not valid. Valid work modes are LucentAgent.MODE_NONE, LucentAgent.MODE_AUTO_IN and " + "LucentAgent.MODE_MANUAL_IN");
     }
     ...
     if (this.tsAgent.getTSProviderImpl().isLucentV7())
     {
       ...
       {
         throw new TsapiInvalidArgumentException(3, 0, "reasonCode not valid - must be in range 0-99 [" + reasonCode + "]");
       }
 
     }
     else if ((reasonCode < 0) || (reasonCode > 9))
     {
       throw new TsapiInvalidArgumentException(3, 0, "reasonCode not valid - must be in range 0-9 [" + reasonCode + "]");
     }
 
     boolean done = this.tsAgent.setState(state, workMode, reasonCode, enablePending);
   ...
   public final AgentTerminal getAgentTerminal()
   {
     ...
     if (tsDevice != null)
     {
       AgentTerminal term = (AgentTerminal)TsapiCreateObject.getTsapiObject(tsDevice, false);
