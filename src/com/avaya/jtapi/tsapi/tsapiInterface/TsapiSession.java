   ...
   public void startSession(String tlink, String login, String passwd, Vector<TsapiVendor> vendors, int timeout)
   {
     try
     {
       ...
       if (event.getEvent() instanceof ACSKeyReply)
       {
         ...
         cryptPass = Crypt.scramblePassword(passwd, reply.getObjectID(), reply.getKey());
       }
       ...
       req = new ACSOpenStream((isRequestingTrustedApplicationStatus()) ? (short)5 : (short)1, tlink, login, cryptPass, "Jtapi Client", (short)1, "TS1:2", "AES5.2.0.483", "");
 
       CSTAPrivate openPriv = null;
 
       String version_range = "4-8";
 
       StringBuffer vendStr = new StringBuffer("#ECS#" + version_range + "#" + "AT&T Definity G3" + "#" + version_range);
...
continue;
       ...
       {
         TSErrorMap.throwACSException(((ACSUniversalFailureConfEvent)event.getEvent()).getError());
       }
       ...
       this.apiVersion = ((ACSOpenStreamConfEvent)event.getEvent()).getApiVer();
     ...
     throws TsapiInvalidStateException, TsapiInvalidArgumentException, TsapiProviderUnavailableException, TsapiInvalidPartyException, TsapiPrivilegeViolationException, TsapiResourceUnavailableException
   {
     ...
     if (sync)
     {
       ...
       {
         TSErrorMap.throwCSTAException(((CSTAUniversalFailureConfEvent)conf.getEvent()).getError());
       }
       if (conf.getEvent() instanceof ACSUniversalFailureConfEvent)
       {
         TSErrorMap.throwACSException(((ACSUniversalFailureConfEvent)conf.getEvent()).getError());
       }
     ...
     throws IOException
   {
     ...
     {
       IntelByteArrayOutputStream acBlock = new IntelByteArrayOutputStream(18);
       IntelByteArrayOutputStream encodeStream = new IntelByteArrayOutputStream();
       IntelByteArrayOutputStream privateData = new IntelByteArrayOutputStream((priv != null) ? 34 + priv.data.length : 0);
 
       log.info("Sent InvokeID " + req.getInvokeID() + " for " + this.debugID);
       ...
       this.out.writeInt(acBlock.size() + encodeStream.size() + privateData.size());
   ...
   public void handleRead(IntelByteArrayInputStream msg)
   {
     try
     {
       ...
       ACSEventHeader eventHeader = new ACSEventHeader(eventClass, eventType);
       ...
       if (pdu instanceof CSTAUnsolicited)
       {
         ...
         log.info("Received monitorCrossRefID " + monitorCrossRefID + " for " + this.debugID);
       }
       else if (pdu instanceof CSTAConfirmation)
       {
         ((CSTAConfirmation)pdu).setInvokeID(invokeID);
         log.info("Received invokeID " + invokeID + " for " + this.debugID);
       }
   ...
   private void processEvent(CSTAEvent event)
   {
     ...
     case 2:
       ...
       if (invokeID == null)
         return;
       if ((invokeID.getConfHandler() != null) && (!(invokeID.getConfHandler() instanceof HandleConfOnCurrentThread)))
       {
         this.eventHandler.handleEvent(event); return;
       }
     ...
     case 6:
       ...
       break;
     default:
       log.info("WARNING: event class " + event.getEventHeader().getEventClass() + " not implemented");
     }
   }
   ...
   public void handleException(Exception e)
   {
     ...
     if (this.inService)
     {
       try
       {
         TsapiSocketException tse = new TsapiSocketException(4, 0, "read request failed");
