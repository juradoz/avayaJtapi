   ...
   implements TsapiChannel
 {
   ...
   private static Socket trySocket(InetSocketAddress addr, SocketFactory sf)
   {
     ...
     {
       throw new TsapiPlatformException(4, 0, "address <" + addr + "> not found");
     }
     catch (IOException e)
     {
       throw new TsapiPlatformException(4, 0, "connection to address <" + addr + "> timed out");
     }
   }
     ...
     public void run() {
       while (this.keepRunning)
         try
         {
           ...
           IntelByteArrayInputStream msg = new IntelByteArrayInputStream(msgBuf);
