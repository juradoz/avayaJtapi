 ...
 class TLSServerCertificateValidator
 {
     ...
     throws CertificateException
   {
     ...
     this.certificates = ((X509Certificate[])(X509Certificate[])peerCertificates);
   ...
   public void validateCommonName()
     throws CertificateException
   {
     Collection altNames = this.certificate.getSubjectAlternativeNames();
     String commonName = "";
   ...
   private String getNameFromX509(X509Certificate certificate)
   {
     ...
     while (tokenizer.hasMoreTokens())
     {
       ...
       if (token.startsWith("CN="))
       {
         if (token.endsWith(","))
         {
           commonName = token.substring(3, token.length() - 1); break;
         }
     ...
     throws CertificateException
   {
     ...
     log.info("Verifying that the certificate's common name \"" + commonName + " matches the peer's hostname.");
 
     if (address.isUnresolved())
     {
       throw new CertificateException("Unable to validate peer certificate: " + address + " could not be resolved to a host name.");
     }
 
     if (address.getHostName().equalsIgnoreCase(commonName))
       return;
     throw new CertificateException("The Common Name (CN) in the server's certificate (" + commonName + ") does not match the server's resolved host name (" + address.getHostName() + ").");
   }
