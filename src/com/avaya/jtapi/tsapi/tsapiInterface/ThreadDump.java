 ...
 public class ThreadDump extends Thread
 {
   ...
   public static void dumpJavaThreadsByAPI()
   {
     ...
     for (int i = 0; i < threads.length; ++i) {
       ...
       String heading = t.getName() + ": " + daemon + " prio=" + t.getPriority() + " tid=" + t.getId() + " state=" + t.getState() + " " + alive + " " + interrupted;
       log.info(heading);
       StackTraceElement[] stes = (StackTraceElement[])map.get(t);
       for (int j = 0; j < stes.length; ++j) {
         StackTraceElement ste = stes[j];
         String line = ste.getClassName() + "." + ste.getMethodName() + "(" + ste.getFileName() + ":" + ste.getLineNumber() + ")";
