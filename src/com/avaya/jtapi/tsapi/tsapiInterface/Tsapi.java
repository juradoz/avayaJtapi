 ...
 public class Tsapi
 {
   ...
   public void init(String tlink, String login, String passwd, Vector<TsapiVendor> vendors, TsapiUnsolicitedHandler handler)
   {
     ...
     try
     {
       ...
       validate(servers);
       ACSNameAddr nameAddr = sessionFac.findTlink(tlink, servers, useTLinkIP);
     ...
     while (eprop.hasMoreElements())
     {
       ...
       else if (!handleVolatileConfigurationUpdate(tsapiProperty, prop))
       {
         if (tsapiProperty.equalsIgnoreCase("useTlinkIP")) {
           try
           {
             int temp = Integer.parseInt(prop.getProperty(tsapiProperty, "0"));
             if (temp == 1) {
               useTLinkIP = true;
             }
         ...
         else if (tsapiProperty.equalsIgnoreCase("enable_PreserveRedirectedVDNs")) {
           try
           {
             int temp = Integer.parseInt(prop.getProperty(tsapiProperty, "0"));
           ...
           try
           {
             maxTcpSocketWait = Integer.parseInt(prop.getProperty(tsapiProperty, "0"));
           }
           catch (Exception re) {
             maxTcpSocketWait = 0;
           }
         }
         else if ((!tsapiProperty.equalsIgnoreCase("trustStoreLocation")) && (!tsapiProperty.equalsIgnoreCase("trustStorePassword"))) if (!tsapiProperty.equalsIgnoreCase("verifyServerCertificate"))
           {
             if (tsapiProperty.regionMatches(true, 0, "Alternates", 0, 10))
             {
               String valueString = prop.getProperty(tsapiProperty);
               if (valueString == null)
               {
                 if (JtapiUtils.isLog4jConfigured())
                   log.info("Error parsing property \"" + tsapiProperty + "\"; " + "could not read property value.");
                 else
                   System.out.println("Error parsing property \"" + tsapiProperty + "\"; " + "could not read property value.");
               }
               else
               {
                 TsapiAlternateTlinkEntriesList alternateTlinkEntriesList = TsapiAlternateTlinkEntriesList.Instance();
                 try
                 {
                   alternateTlinkEntriesList.addAlternateTlinkEntry(tsapiProperty, valueString);
                 }
...
continue;
               ...
               try
               {
                 port = Integer.parseInt(prop.getProperty(tsapiProperty, Integer.toString(450)).trim());
               }
               catch (Exception re2)
               {
                 if (JtapiUtils.isLog4jConfigured()) {
                   log.error("Invalid name value pair in : " + tsapiProperty);
                 }
                 else {
                   System.out.println("Invalid name value pair in : " + tsapiProperty);
                 }
                 port = 450;
               }
   ...
   private static void displayProperties(Properties systemProperties, Properties jtapiProperties)
   {
     ...
     for (Object entry : list)
     {
       ...
       else if ((key.equals("maxWaitForSocket")) || (key.equals("propertyRefreshRate")) || (key.equals("callCompletionTimeout")) || (key.equals("callCleanupRate")))
       {
         value = value + " seconds";
       }
   ...
   private static int getIntegerProperty(String tsapiProperty, Properties props, String defaultValue, int currentValue)
   {
     ...
     try
     {
       ...
       {
         log.info("Property \"" + tsapiProperty + "\" set to " + returnValue);
       }
     }
     catch (NumberFormatException ee)
     {
       if (JtapiUtils.isLog4jConfigured()) {
         log.error("Invalid integer value " + propertyValue + " given for property " + tsapiProperty);
         log.error(ee.getMessage(), ee);
       } else {
         System.out.println("Invalid integer value " + propertyValue + " given for property " + tsapiProperty);
         ee.printStackTrace();
       }
     }
     ...
     throws IOException
   {
     ...
     {
       JTAPILoggingAdapter.setTraceLoggerLevel(prop.getProperty(tsapiProperty));
     }
     else if (tsapiProperty.equalsIgnoreCase("getServicesTimeout")) {
       int value = getIntegerProperty("getServicesTimeout", prop, "10", getServicesTimeout);
 
       getServicesTimeout = value * 1000;
     }
     else if (tsapiProperty.equalsIgnoreCase("callCleanupRate")) {
       int value = getIntegerProperty("callCleanupRate", prop, "100", callCleanupRate);
       int roundedOfValue = 0;
       if (value < 10) {
         roundedOfValue = 10;
         if (JtapiUtils.isLog4jConfigured())
           log.info("value specified for property: callCleanupRate is " + value + ". Rounding up to multiple of 10. Final value = " + roundedOfValue);
         else
           System.out.println("value specified for property: callCleanupRate is " + value + ". Rounding up to multiple of 10. Final value = " + roundedOfValue);
       }
       else if (value % 10 != 0) {
         if (value % 10 < 5)
       ...
       if (value != roundedOfValue)
         prop.setProperty(tsapiProperty, Integer.toString(roundedOfValue));
       callCleanupRate = roundedOfValue;
     } else if (tsapiProperty.equalsIgnoreCase("callCompletionTimeout")) {
       int value = getIntegerProperty("callCompletionTimeout", prop, "15", callCompletionTimeout / 1000);
     ...
     else if (tsapiProperty.equalsIgnoreCase("enableAuditDump"))
     {
       ...
       if ((propertyValue == null) || ((!propertyValue.equalsIgnoreCase(Boolean.FALSE.toString())) && (!propertyValue.equalsIgnoreCase(Boolean.TRUE.toString()))))
       {
         if (JtapiUtils.isLog4jConfigured())
       ...
       else isEnableAuditDump = Boolean.parseBoolean(propertyValue);
 
     }
     else if (tsapiProperty.equalsIgnoreCase("auditDumpInterval"))
     {
       auditDumpInterval = getIntegerProperty("auditDumpInterval", prop, "3", auditDumpInterval);
     }
     else if (tsapiProperty.equalsIgnoreCase("auditObjectAgeThreshold"))
     {
       auditObjectAgeThreshold = getIntegerProperty("auditObjectAgeThreshold", prop, "60", auditObjectAgeThreshold);
     }
     else if (tsapiProperty.equalsIgnoreCase("propertyRefreshRate"))
     {
       int newValue = Integer.parseInt(prop.getProperty(tsapiProperty, "100"));
     ...
     {
       JTAPILoggingAdapter.setAltTraceFile(prop.getProperty(tsapiProperty));
       ...
       JTAPILoggingAdapter.setTraceFileCount(prop.getProperty(tsapiProperty));
     } else if (tsapiProperty.equalsIgnoreCase("traceFileSize")) {
       JTAPILoggingAdapter.setTraceFileSize(prop.getProperty(tsapiProperty));
       ...
       JTAPILoggingAdapter.setErrorFileCount(prop.getProperty(tsapiProperty));
     } else if (tsapiProperty.equalsIgnoreCase("errorFileSize")) {
       JTAPILoggingAdapter.setErrorFileSize(prop.getProperty(tsapiProperty));
       ...
       JTAPILoggingAdapter.setPerfFileCount(prop.getProperty(tsapiProperty));
     } else if (tsapiProperty.equalsIgnoreCase("perfFileSize")) {
       JTAPILoggingAdapter.setPerfFileSize(prop.getProperty(tsapiProperty));
     } else if (tsapiProperty.equalsIgnoreCase("performanceWindow")) {
       PerfStatisticsCollector.setPerformanceWindow(Integer.parseInt(prop.getProperty(tsapiProperty)));
     } else if (tsapiProperty.equalsIgnoreCase("unsolicitedHandlingTimeThreshold")) {
       PerfStatisticsCollector.setUnsolicitedHandlingTimeThreshold(Long.parseLong(prop.getProperty(tsapiProperty)));
     } else if (tsapiProperty.equalsIgnoreCase("serviceRequestTurnaroundTimeThreshold")) {
       PerfStatisticsCollector.setServiceRequestTurnaroundTimeThreshold(Long.parseLong(prop.getProperty(tsapiProperty)));
     } else if (tsapiProperty.equalsIgnoreCase("queueLengthThreshold")) {
       PerfStatisticsCollector.setQueueLengthThreshold(Long.parseLong(prop.getProperty(tsapiProperty)));
     } else if (tsapiProperty.equalsIgnoreCase("messageLatencyThreshold")) {
       PerfStatisticsCollector.setMessageLatencyThreshold(Long.parseLong(prop.getProperty(tsapiProperty)));
     } else if (tsapiProperty.equalsIgnoreCase("maxThreadPoolSize")) {
       try {
         maxThreadPoolSize = Integer.parseInt(prop.getProperty(tsapiProperty, "20"));
       }
       catch (Exception e)
       {
         if (JtapiUtils.isLog4jConfigured())
           log.error("Invalid value encountered for " + tsapiProperty + ". Setting to default value", e);
         else {
           System.out.println("Invalid value encountered for " + tsapiProperty + ". Setting to default value." + e.getMessage());
         }
       }
     }
     else if (tsapiProperty.equalsIgnoreCase("tsDevicePerformanceOptimization"))
     {
       String propertyValue = prop.getProperty(tsapiProperty, "false");
 
       tsDevicePerformanceOptimization = Boolean.valueOf(propertyValue).booleanValue();
     }
     else if (tsapiProperty.equalsIgnoreCase("handleCstaEventTimeThreshold"))
     {
       int threshold;
       try
       {
         threshold = Integer.parseInt(prop.getProperty(tsapiProperty, "250"));
       }
       catch (Exception re)
       {
       ...
       {
         if (JtapiUtils.isLog4jConfigured())
           log.info("Requested setting for property \"handleCstaEventTimeThreshold\" (" + threshold + ") is invalid.");
         else {
           System.out.println("Requested setting for property \"handleCstaEventTimeThreshold\" (" + threshold + ") is invalid.");
         }
         threshold = Integer.parseInt("250");
       }
   ...
   public static void updateVolatileConfigurationValues()
   {
     try
     {
       ...
       while (eprop.hasMoreElements())
       {
         handleVolatileConfigurationUpdate((String)eprop.nextElement(), prop);
       }
   ...
   public CSTAEvent monitorCallsViaDevice(String deviceID, CSTAMonitorFilter monitorFilter, CSTAPrivate priv)
     throws TsapiInvalidStateException, TsapiInvalidArgumentException, TsapiProviderUnavailableException, TsapiInvalidPartyException, TsapiPrivilegeViolationException, TsapiResourceUnavailableException
   {
     TsapiRequest req = new CSTAMonitorCallsViaDevice(deviceID, monitorFilter);
     return this.session.send(req, priv);
   }
   ...
   public void setAgentState(String agentDevice, short agentMode, String agentID, String acdGroup, String agentPassword, CSTAPrivate priv, ConfHandler handler)
     throws TsapiInvalidStateException, TsapiInvalidArgumentException, TsapiProviderUnavailableException, TsapiInvalidPartyException, TsapiPrivilegeViolationException, TsapiResourceUnavailableException
   {
     TsapiRequest req = new CSTASetAgentState(agentDevice, agentMode, agentID, acdGroup, agentPassword);
     this.session.send(req, priv, handler);
   }
   ...
   public void setFwd(String device, short forwardingType, boolean forwardingOn, String forwardingDestination, CSTAPrivate priv, ConfHandler handler)
     throws TsapiInvalidStateException, TsapiInvalidArgumentException, TsapiProviderUnavailableException, TsapiInvalidPartyException, TsapiPrivilegeViolationException, TsapiResourceUnavailableException
   {
     CSTAForwardingInfo forward = new CSTAForwardingInfo(forwardingType, forwardingOn, forwardingDestination);
   ...
   public void makePredictiveCall(String callingDevice, String calledDevice, short allocationState, CSTAPrivate priv, ConfHandler handler)
     throws TsapiInvalidStateException, TsapiInvalidArgumentException, TsapiProviderUnavailableException, TsapiInvalidPartyException, TsapiPrivilegeViolationException, TsapiResourceUnavailableException
   {
     TsapiRequest req = new CSTAMakePredictiveCall(callingDevice, calledDevice, allocationState);
   ...
   public void selectRoute(int routeRegisterReqID, int routingCrossRefID, String routeSelected, int remainRetry, String isdnSetupMessage, boolean routeUsedReq, CSTAPrivate priv)
   {
     ...
     {
       req = new CSTARouteSelectRequest(routeRegisterReqID, routingCrossRefID, routeSelected, remainRetry, isdnBuf, routeUsedReq);
     }
     else
     {
       req = new CSTARouteSelectRequestInv(routeRegisterReqID, routingCrossRefID, routeSelected, remainRetry, isdnBuf, routeUsedReq);
     }
   ...
   public void routeEnd(int routeRegisterReqID, int routingCrossRefID, short errorValue, CSTAPrivate priv)
   {
     ...
     if (this.session.getApiVersion().equals("1"))
     {
       req = new CSTARouteEndRequest(routeRegisterReqID, routingCrossRefID, errorValue);
     }
     else
     {
       req = new CSTARouteEndRequestInv(routeRegisterReqID, routingCrossRefID, errorValue);
     }
   ...
   static
   {
     ...
     try
     {
       if (System.getProperty("com.avaya.jtapi.tsapi.servers") != null) {
         Collection serverEntries = JtapiUtils.parseTelephonyServerEntry(System.getProperty("com.avaya.jtapi.tsapi.servers").trim(), 450);
     ...
     catch (Exception e)
     {
       try
       {
         ...
         String hostname = TsapiChannelOio.getBrowser().getCodeBaseServer();
