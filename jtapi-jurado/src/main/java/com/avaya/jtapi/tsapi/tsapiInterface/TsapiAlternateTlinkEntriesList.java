...
public final class TsapiAlternateTlinkEntriesList {
	static final int MAX_TLINK_ALTERNATE_ENTRIES = 16;
	...
	private TsapiAlternateTlinkEntriesList() {
		this.entries = new ArrayList<TsapiAlternateTlinkEntry>();
	}
	...
	public synchronized void addAlternateTlinkEntry(String propertyName,
		...
		try {
			...
			this.entries.add(new TsapiAlternateTlinkEntry(propertyName,
					valueString));
	...
	public synchronized int getAlternateTlinkIndex(String preferredTlinkName,
			String tlinkName) {
		try {
			Iterator<TsapiAlternateTlinkEntry> entryIterator = this.entries.iterator();
