...
final class ConfXferConfHandler implements ConfHandler {
	...
	public void handleConf(CSTAEvent event) {
		...
		switch (this.pdu) {
		case 12:
			this.newCall = ((CSTAConferenceCallConfEvent) event.getEvent())
					.getNewCall();
			connList = ((CSTAConferenceCallConfEvent) event.getEvent())
					.getConnList();
			cause = 207;
			if ((event.getPrivData() instanceof LucentConferenceCallConfEvent)) {
				this.call.setUCID(((LucentConferenceCallConfEvent) event
						.getPrivData()).getUcid());
			}
			break;
		case 52:
			this.newCall = ((CSTATransferCallConfEvent) event.getEvent())
					.getNewCall();
			connList = ((CSTATransferCallConfEvent) event.getEvent())
					.getConnList();
			cause = 212;
			if ((event.getPrivData() instanceof LucentTransferCallConfEvent)) {
				this.call.setUCID(((LucentTransferCallConfEvent) event
						.getPrivData()).getUcid());
			}
			break;
		case 90:
			if ((event.getPrivData() instanceof LucentSingleStepConferenceCallConfEvent)) {
				this.newCall = ((LucentSingleStepConferenceCallConfEvent) event
						.getPrivData()).getNewCall();
				connList = ((LucentSingleStepConferenceCallConfEvent) event
						.getPrivData()).getConnList();
				cause = 207;
				this.call
						.setUCID(((LucentSingleStepConferenceCallConfEvent) event
								.getPrivData()).getUcid());
			} else if ((event.getPrivData() instanceof LucentSingleStepTransferCallConfEvent)) {
				this.newCall = ((LucentSingleStepTransferCallConfEvent) event
						.getPrivData()).getTransferredCall();
				connList = null;
				cause = 212;

				if ((event.getPrivData() instanceof LucentV9SingleStepTransferCallConfEvent)) {
					this.call
							.setUCID(((LucentV9SingleStepTransferCallConfEvent) event
									.getPrivData()).getUcid());
				}
			} else {
				return;
		...
		if (connList == null) {
			...
			if (this.call.getTSProviderImpl().getConnection(this.newCall) == null) {
				TSConnection sstDestConnection = createNewCallConnectionTryToGetStateFromOtherCall(
						this.call, this.otherCall, this.newCall, priEventList,
						snapConnections);
				...
				if (device != null) {
					...
					if (foundTSConn != null) {
						try {
							...
							log.error("TSCall.handleConf() caught TsapiPlatformException from setConnID() while processing connList, i="
									+ i + ", party=" + connList[i].getParty());
						...
						if (foundTSConn.isTerminalConnection()) {
							...
							if (tcs == 96) {
								if (this.call.getTSProviderImpl()
										.getCapabilities().getSnapshotCallReq() != 0) {
						...
						conn = createNewCallConnectionTryToGetStateFromOtherCall(
								this.call, this.otherCall,
								connList[i].getParty(), priEventList,
								snapConnections);
		...
		if (snapConnections.size() > 0) {
			...
			SnapshotCallExtraConfHandler handler = new XferConfSnapshotCallConfHandler(
					this.call, cause, null, snapConnections);

			this.call.doSnapshot(
