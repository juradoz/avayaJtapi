...
final class TsapiSSLContext {
	...
	private TsapiSSLContext(String trustStoreLocation, String trustStorePassword) {
		...
		try {
			...
			log.info("TrustStore algorithm being used is  \"" + algorithm
					+ "\"");
		...
		try {
			...
			if (tsFile.isAbsolute()) {
				...
				while (classpath.hasMoreTokens()) {
					...
					if (tsFile.canRead()) {
						log.info("Classpath search for trust store \""
	...
	private static void setupServerTrustStore(String trustStoreFile,
		...
		FileInputStream jksInputStream = null;
		try {
			jksInputStream = new FileInputStream(jksFile);

			serverTrustStore = KeyStore.getInstance("JKS");
			serverTrustStore.load(jksInputStream,
			...
			if (jksInputStream != null) {
				try {
					...
					log.error("Error closing file: " + jksFile);
				}
			}
		}
	}
